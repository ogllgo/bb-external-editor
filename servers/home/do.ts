...
export const FILENAME = "do.js";
export const HOSTNAME = "home";
export const BASERAMCOST = 1.6;
export async function outsource<T extends string, Q = GetPath<NS, T>>(ns: NS, func: AutoPath<NS, T>, ...args: Q extends (...args: infer R) => infer _ ? R : never): Promise<Success<T, GetPath<NS, T>> | Awaited<Q extends (...args: infer _) => infer R ? R : never>> {
	const data = await outsourceNoException(ns, func, ...args);
	if (data === null) throw new Error("Outsourcing failed! Got null");
	return data;
}

export async function outsourceNoException<T extends string, Q = GetPath<NS, T>>(ns: NS, func: AutoPath<NS, T>, ...args: Q extends (...args: infer R) => infer _ ? R : never): Promise<(Q extends (...args: infer _) => infer R ? R : never) | null | Success<T, GetPath<NS, T>>> {
    ...
    // this is essentially "if (true)" but typescript doesn't like it if this has no if
...
function isResult<T extends string>(obj: unknown): obj is Result<T> {
